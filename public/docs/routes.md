...
|`/profil/{id}/`|GET|UserController|profileShow|Profil utilisateur|Le profil "public" d'un utilisateur| accessible uniquement par les utilisateurs connectés et non bloqués|
|`/profil/edit/{id}/`|POST|UserController|edit|Mise à jour de son profil utilisateur|MAJ de son profil d'un utilisateur| - |
|`/profil/delete/{id}/`|GET|UserController|delete|Possibilité de supprimer son profil|Suppression en cascade de tout ce qu'il a posté| - |
...
|`backend/{id}/status/{statusCode}/`|POST|Backend\PostController|updateStatus|Modification du status d'un user|possibilité de modifier le statut d'un user|-|
